...
import br.com.pentamc.common.utils.ip.FetchAddressException;
import br.com.pentamc.common.utils.ip.IpFetcher;
...
import br.com.pentamc.bungee.bungee.BungeeClan;
...
//				}
	...
	public boolean checkLogin(PreLoginEvent event) {
		...
		try {
			JsonObject jsonObject = CommonConst.DEFAULT_WEB
					.doRequest(CommonConst.MOJANG_FETCHER + event.getConnection().getName(), WebHelper.Method.GET)
					.getAsJsonObject();
	...
	public boolean loadMember(LoginEvent event) {
		...
		if (member == null) {
			member = new BungeeMember(playerName, event.getConnection().getUniqueId(),
					!event.getConnection().isOnlineMode() ? LoginConfiguration.AccountType.CRACKED : LoginConfiguration.AccountType.ORIGINAL);
			...
			if (member.getLoginConfiguration().getAccountType() == LoginConfiguration.AccountType.CRACKED) {
				if (!member.getPlayerName().equals(playerName)) {
					event.setCancelled(true);
					event.setCancelReason("Sua conta está com nickname diferente do original registrado no servidor!");
					return false;
				}
			}
		}
		...
		if (group != null) {
			if (!member.hasGroupPermission(group)) {
				event.setCancelled(true);
				event.setCancelReason("§cEsta sala está restrita aos membros da equipe.");
				return false;
			}
		}
		...
		if (member.hasSkin()) {
			if (textureCache.asMap().containsKey(playerName))
				...
				CommonConst.DEFAULT_WEB.doAsyncRequest(
						String.format(CommonConst.SKIN_URL, member.getSkinProfile().getUniqueId()), WebHelper.Method.GET,
	...
	public void onIpRemove(IpRemoveEvent event) {
		...
		CommonGeneral.getInstance().getCommonPlatform().runAsync(() -> {
			...
			ipInfo.setIpStatus(IpInfo.IpStatus.ACCEPT);
	...
	public void blockIp(String ipAddress) {
		...
		if (ipInfo.getIpStatus() == IpInfo.IpStatus.ACCEPT)
			ipInfo.setIpStatus(IpInfo.IpStatus.BLOCKED);
