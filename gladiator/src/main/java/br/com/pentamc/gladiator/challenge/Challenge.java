package br.com.pentamc.gladiator.challenge;
...
import br.com.pentamc.gladiator.event.GladiatorPulseEvent;
...
public class Challenge {
    ...
    public void pulse() {
        ...
        if (time > 0 && time % 150 == 0) {
            enimy.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 20 * 60, 5));
            player.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 20 * 60, 5));

            Bukkit.getScheduler().runTaskLater(GameMain.getInstance(), () -> {
    ...
    public void handleInventory(Player player) {
        ...
        player.getInventory().setContents(Custom.getContents(player.getUniqueId()));
