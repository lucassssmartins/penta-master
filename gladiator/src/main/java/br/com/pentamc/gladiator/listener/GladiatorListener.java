...
import br.com.pentamc.bukkit.api.cooldown.types.ItemCooldown;
import br.com.pentamc.bukkit.api.item.ActionItemStack;
import br.com.pentamc.bukkit.api.item.ActionItemStack.ActionType;
import br.com.pentamc.bukkit.api.item.ActionItemStack.Interact;
...
import br.com.pentamc.gladiator.event.GladiatorSpectatorEvent;
import br.com.pentamc.gladiator.event.GladiatorSpectatorEvent.Action;
import br.com.pentamc.gladiator.event.GladiatorStartEvent;
import br.com.pentamc.gladiator.event.GladiatorTrySpectatorEvent;
...
public class GladiatorListener implements Listener {
    ...
    private final ItemStack queueJoin = new ItemBuilder().name("§aGladiator Rápido ").type(Material.INK_SACK)
                                                         .durability(8).build();
    private final ItemStack queueLeave = new ItemBuilder().name("§aGladiator Rápido §7(Procurando...)").type(Material.INK_SACK)
                                                          .durability(10).build();

    private ItemStack oldQueueJoin = new ItemBuilder().name("§aGladiator Ranked").type(Material.ARROW).build();
    private ItemStack oldQueueLeave = new ItemBuilder().name("§aGladiator Ranked §7(Procurando...)").type(Material.ARROW).glow()
                                                       .build();
    ...
    public GladiatorListener() {
        ...
        customItem = new ActionItemStack(
                new ItemBuilder().type(Material.IRON_FENCE).name("§aGladiator Customizado §7(VIPs)").glow().build(),
                new Interact() {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (player.getItemInHand().getType() == Material.INK_SACK) {
            ...
            if (normalQueue.contains(player)) {
                player.sendMessage("§eVocê saiu da fila da §bGladiator Rápido§e!");
                normalQueue.remove(player);
                player.setItemInHand(queueJoin);
            } else {
                if (normalQueue.isEmpty()) {
                    player.sendMessage("§eVocê entrou na fila da §bGladiator Rápido§e!");
                    ...
                    if (challenger.getUniqueId().equals(player.getUniqueId())) {
                        ...
                        challenger.sendMessage("§eVocê está batalhando contra §b" + player.getName() + "§e!");
                        player.sendMessage("§eVocê está batalhando contra §b" + challenger.getName() + "§e!");
                    }
                }
            }
            ...
            if (oldQueue.contains(player)) {
                player.sendMessage("§eVocê saiu da fila da §bGladiator Ranked§e!");
                player.setItemInHand(oldQueueJoin);
                oldQueue.remove(player);
            } else {
                if (oldQueue.isEmpty()) {
                    player.sendMessage("§eVocê entrou na fila da §bGladiator Ranked§e!");
                    oldQueue.add(player);
