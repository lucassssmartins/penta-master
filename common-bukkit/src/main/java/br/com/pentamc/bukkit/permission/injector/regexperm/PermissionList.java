package br.com.pentamc.bukkit.permission.injector.regexperm;
...
import br.com.pentamc.bukkit.permission.injector.FieldReplacer;
...
public class PermissionList extends HashMap<String, Permission> {
	...
	private final Multimap<String, Entry<String, Boolean>> childParentMapping = Multimaps
			.synchronizedMultimap(HashMultimap.<String, Entry<String, Boolean>>create());
	...
	private void removeAllChildren(String perm) {
		for (Iterator<Entry<String, Entry<String, Boolean>>> it = childParentMapping.entries().iterator(); it
				.hasNext();) {
