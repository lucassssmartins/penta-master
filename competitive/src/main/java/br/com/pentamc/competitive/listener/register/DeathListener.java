...
import br.com.pentamc.competitive.event.player.PlayerDeathDropItemEvent;
...
import br.com.pentamc.competitive.menu.spectator.SpectatorInventory;
...
import br.com.pentamc.bukkit.utils.item.ItemUtils;
...
public class DeathListener extends GameListener {
	...
	public void onDeath(PlayerDeathEvent event) {
		...
		CombatListener.CombatLog combatLog = CombatListener.combatMap.get(player.getUniqueId());

		if (combatLog != null) {
			if (combatLog.getCombatLogged() != null) {
				Player target = Bukkit.getPlayer(combatLog.getCombatLogged());
				if (target != null) {
					br.com.pentamc.bukkit.listener.register.CombatListener.setClean(target);
				}
			}
		}
		...
		if (combatLog != null)
			killler = getGameGeneral().getGamerController().getGamer(combatLog.getCombatLogged());

		if (killler != null) {
			Member battleKiller = CommonGeneral.getInstance().getMemberManager().getMember(killler.getUniqueId());
		...
		if (player.getKiller() != null) {
			...
			if (battleKiller != null) {
				...
				battleKiller.addXp(StatusType.HG, winnerXp);
		...
		if (ServerConfig.getInstance().isRespawnEnabled()
			...
			replaces.put("%player_Kit%", NameUtils.formatString(gamer.getKitName(KitType.PRIMARY)));
			...
			if (killer != null) {
				...
				replaces.put("%killed_By_Kit%", NameUtils.formatString(killerGamer.getKitName(KitType.PRIMARY)));
